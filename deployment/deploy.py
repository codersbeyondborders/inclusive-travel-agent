"""Deployment script for Inclusive Travel Agent."""
...
def create(env_vars: dict[str, str]) -> None:
    ...
    remote_agent = agent_engines.create(  
        app,
        display_name="Inclusive-Travel-Agent-ADK",
...
def main(argv: list[str]) -> None:
    ...
    # Variables for Inclusive Travel Agent from .env
    initial_states_path = (
        FLAGS.initial_states_path
        if FLAGS.initial_states_path
        else os.getenv("INCLUSIVE_TRAVEL_AGENT_SCENARIO")
    )
    env_vars["INCLUSIVE_TRAVEL_AGENT_SCENARIO"] = initial_states_path
    map_key = (
        FLAGS.initial_states_path
        if FLAGS.initial_states_path
    ...
    if not project_id:
        ...
        print("Missing required environment variable: INCLUSIVE_TRAVEL_AGENT_SCENARIO")
        return
    elif not map_key:
